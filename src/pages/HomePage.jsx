...
const HomePage = () => {
  ...
  const handleFilter = () => {
    if (filterCode === '') {
      ...
      const filteredProduct = allProducts.find(product => product.id === Number(filterCode));
      setDisplayedProducts(filteredProduct ? [filteredProduct] : []);
    }
  };
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <div style={filterContainerStyle}>
        ...
        />
        <button onClick={handleFilter} style={filterButtonStyle}>Filtrar</button>
      </div>
          ...
          <li key={product.id} style={productCardStyle}>
            <div style={imageContainerStyle}>
              <div style={circularImageStyle}>
                <img src={product.pictureUrl} alt={product.name} style={imageStyle} />
              </div>
            </div>
            ...
            <div style={infoContainerStyle}>
              ...
              <div style={{ marginTop: '20px' }}>
                <span style={{ fontWeight: 'bold', fontSize: '1.2rem' }}>R$ {product.price.toFixed(2).replace('.', ',')}</span>
              </div>
            </div>
...
const productCardStyle = {
  ...
  marginBottom: '16px',
  position: 'relative',
  border: '1px solid #e0e0e0', // Borda fina adicionada aqui
};
